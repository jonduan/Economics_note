...
Sys.setenv(CM_API_KEY ='CensusMapper_2f7168cb4fae0a6d646e8bffbf3e2ed3')
```
...
# Find the information by DT Table
...
# Find the information using JSON
...
census_data <- get_census(dataset='CA16', regions=list(CMA="59933"),
                          vectors=c("v_CA16_408","v_CA16_409","v_CA16_410", "v_CA16_2397"),
                          level='CSD')
```
...
library(cancensus)
library(sf)
# retrieve sf dataframe "median_hh_income"=
toronto <- get_census(dataset='CA16', regions=list(CMA="35535"),
                         vectors=c("v_CA16_2397"), level='CSD', quiet = TRUE, 
...
```{r}
library("dplyr")
toronto <- rename(toronto,  median_hh_income = "v_CA16_2397")
```
...
```{r}
toronto[toronto$median_hh_income > 100000,14]
```


```{r}
plot(toronto[toronto$median_hh_income > 100000,14], main = "Toronto Household Income by CSD")
```
...
# plot(toronto[toronto$median_hh_income > 5000,14], col = "red", add = TRUE)
plot(toronto[toronto$median_hh_income > 100000,14],add = TRUE)
